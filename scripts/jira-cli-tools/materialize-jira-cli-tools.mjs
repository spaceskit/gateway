...
import {
  ...
  buildJiraCliManifest,
  buildJiraCliToolReadme,
  getJiraToolDefinitionByOperation,
  resolveDefaultSpacesJiraWrapperPath,
} from "./catalog.mjs";

export { resolveDefaultSpacesJiraWrapperPath } from "./catalog.mjs";

export async function materializeJiraCliTools(input) {
  const targetDir = requireAbsoluteOrResolve(input?.targetDir, "targetDir");
  const wrapperPath = requireAbsoluteOrResolve(
    input?.wrapperPath ?? resolveDefaultSpacesJiraWrapperPath(),
    "wrapperPath",
  );
  ...
  const tools = [];
  for (const tool of JIRA_TOOL_DEFINITIONS) {
    ...
    const manifest = buildJiraCliManifest(tool, { wrapperPath, fixedCwd, enabled: existingEnabled });
    const readme = buildJiraCliToolReadme(tool);
...
async function readExistingEnabled(manifestPath) {
  try {
    ...
    // Keep the default enabled state for fresh or unreadable manifests.
  }
  return true;
}
...
function normalizeToolIdFilter(values) {
  ...
  for (const value of values) {
    ...
    const byId = JIRA_TOOL_DEFINITIONS.find((tool) => tool.id === raw);
    if (byId) {
      normalized.add(byId.id);
      continue;
    }
    const byOperation = getJiraToolDefinitionByOperation(raw);
    if (byOperation) {
      normalized.add(byOperation.id);
      continue;
    }
    throw new Error(`Unknown Jira tool id or operation: ${raw}`);
  }
  return normalized;
}
...
function buildHelpText() {
  return [
    "Materialize Spaces Jira CLI tools",
    "",
    "Usage:",
    "  materialize-jira-cli-tools --target <gateway-cli-tools-dir> [--wrapper <path>] [--fixed-cwd <path>] [--tool <id-or-operation>]",
    "  materialize-jira-cli-tools --version",
    "",
    "Examples:",
    "  materialize-jira-cli-tools --target ~/spaces/gateway/cli-tools",
    "  materialize-jira-cli-tools --target ~/spaces/gateway/cli-tools --tool jira.issue.view --tool sprint.close",
  ].join("\n");
}
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (parsed.version) {
    process.stdout.write(`materialize-jira-cli-tools ${SPACES_JIRA_WRAPPER_VERSION}\n`);
    return;
  }
  if (!parsed.targetDir) {
    throw new Error("The --target option is required.");
  }
  const result = await materializeJiraCliTools(parsed);
  process.stdout.write(
    `Materialized ${result.toolCount} Jira CLI tool bundle(s) into ${result.targetDir}\n`,
  );
}
