...
import {
  ...
  buildPeekabooCliManifest,
  buildPeekabooCliToolReadme,
  getPeekabooToolDefinitionByOperation,
  resolveDefaultSpacesPeekabooWrapperPath,
} from "./catalog.mjs";

export { resolveDefaultSpacesPeekabooWrapperPath } from "./catalog.mjs";

export async function materializePeekabooCliTools(input) {
  ...
  for (const tool of PEEKABOO_TOOL_DEFINITIONS) {
    ...
    const manifest = buildPeekabooCliManifest(tool, { wrapperPath, fixedCwd, enabled: existingEnabled });
    const readme = buildPeekabooCliToolReadme(tool);
...
function normalizeToolIdFilter(values) {
  ...
  for (const value of values) {
    ...
    const byId = PEEKABOO_TOOL_DEFINITIONS.find((tool) => tool.id === raw);
    if (byId) {
      normalized.add(byId.id);
      continue;
    }
    const byOperation = getPeekabooToolDefinitionByOperation(raw);
    if (byOperation) {
      normalized.add(byOperation.id);
      continue;
    }
    throw new Error(`Unknown Peekaboo tool id or operation: ${raw}`);
  }
  return normalized;
}
...
function buildHelpText() {
  return [
    "Materialize Spaces Peekaboo CLI tools",
    "",
    "Usage:",
    "  materialize-peekaboo-cli-tools --target <gateway-cli-tools-dir> [--wrapper <path>] [--fixed-cwd <path>] [--tool <id-or-operation>]",
    "  materialize-peekaboo-cli-tools --version",
  ].join("\n");
}
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (parsed.version) {
    process.stdout.write(`materialize-peekaboo-cli-tools ${SPACES_PEEKABOO_WRAPPER_VERSION}\n`);
    return;
  }
  if (!parsed.targetDir) {
    throw new Error("The --target option is required.");
  }
  const result = await materializePeekabooCliTools(parsed);
  process.stdout.write(
    `Materialized ${result.toolCount} Peekaboo CLI tool bundle(s) into ${result.targetDir}\n`,
  );
}
