...
export interface GatewayWorkspaceDefaultsRow {
  singleton_id: number;
  space_home_root_override: string;
  updated_at: string;
}
...
export class GatewayWorkspaceDefaultsRepository {
  ...
  setSpaceHomeRootOverride(spaceHomeRootOverride: string): GatewayWorkspaceDefaultsRow {
    const now = new Date().toISOString();
    this.db.query(`
      INSERT INTO gateway_workspace_defaults(
        singleton_id,
      ...
      ON CONFLICT(singleton_id) DO UPDATE SET
        space_home_root_override = excluded.space_home_root_override,
        updated_at = excluded.updated_at
    `).run(spaceHomeRootOverride, now);
