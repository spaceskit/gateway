...
function buildPlanningPrompt(
  ...
  return [
    ...
    "Keep the North Star runner loop fixed: pick task, run task, watch progress, see evidence, then review, retry, or cancel.",
    "The external harness (~/Documents/work/harness) is this task's SOURCE: it captured, enriched, and ranked the task file. The Workbench is the RUNNER. Read the task file and any harness-provided context paths freely, but execute through the Spaces-native Workbench protocol — do not invoke harness binaries to perform, re-enrich, or close out the work.",
    "",
...
function buildImplementationPrompt(
  ...
  return [
    ...
    "Follow the task file and the planning discussion. Work autonomously within the existing review gate; do not merge, commit, push, or land changes unless the operator explicitly approves it.",
    "Report changed files and implementation evidence in the execution Space. Treat Workbench verification output as canonical, even if local intuition disagrees.",
    "Harness tooling prepared this task; the Workbench runs it and owns status write-back (in-progress/review/blocked — never done/dropped). Do not call harness binaries to execute or close the task. If some other runner or CLI looks useful, note it as a future executor adapter candidate rather than adding it as a dependency during this run.",
    "",
