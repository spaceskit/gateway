...
describe("OrchestratorCommandService control-plane commands", () => {
  test("lists rooms through list_rooms", async () => {
    const db = initDatabase({
      path: ":memory:",
      runtimeGeneration: `test-orchestrator-command-${crypto.randomUUID()}`,
    });
    ...
    expect(result.status).toBe("completed");
    expect((result.result?.rooms as Array<Record<string, unknown>>).length).toBe(2);
  });
  ...
  test("handoff_room resolves room and optionally dispatches prompt", async () => {
    const db = initDatabase({
      path: ":memory:",
      runtimeGeneration: `test-orchestrator-command-${crypto.randomUUID()}`,
    });
    ...
    const result = await service.submitCommand({
      commandType: "handoff_room",
      targetSpaceId: "space-main",
      payload: {
        handoffSpaceId: "space-design",
        promptText: "continue in the design room",
        targetAgentId: "agent-room",
      },
    });
    ...
    expect(executeTurnCalls).toEqual([
      {
        ...
        input: "continue in the design room",
        targetAgentId: "agent-room",
      },
    ]);
  });
